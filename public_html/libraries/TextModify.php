...
class TextModify
{
    ...
    public function translit($str)
    {
        ...
        if($link)
        {
            $link = preg_replace('/[^a-z0-9_-]/iu', '', $link); //i нижний регистр, u - мультибайтовая кодировка, ^ в [] - означает "НЕ" эти символы
            $link = preg_replace('/-{2,}/iu', '-', $link);      // "-{2,}" - дифиз два и более раз
            $link = preg_replace('/_{2,}/iu', '_', $link);
            $link = preg_replace('/(^[_-]+?)|([_-]+$)/iu', '', $link); // "(^)" - с начала строки, "|" - или, + сколько угодно раз, $ - в конце строки
        }
        return $link;
    }
}