...
class CreatesitemapController extends BaseAdmin
{
    ...
    {
        $links_counter  = $this->cleanNum($links_counter);
        ...
        {
            $temp_link_count    = count($this->temp_links);
            $links              = $this->temp_links;
            $this->temp_links   = [];

            if ($temp_link_count > $this->maxLinks)
            {
                $links  = array_chunk($links, ceil($temp_link_count / $this->maxLinks));
    ...
    protected function checkParsingTable()
    {
        ...
        if (!in_array('parsing_data', $tables))
        {
            ...
            if (!$this->model->query($query, 'c') || !$this->model->add('parsing_data', [
                'fields' => [
                    'all_links' => '',
                    'temp_links' => '',
                    'bad_links' => ''
    ...
    protected function createLinks($content)
    {
        if ($content)
        {
            //'<a class="class" id="1" href="dfgfdgdfgfgdf" data-id="sdfdsfsd">';
            /**'/<a\s*?[^>]*?href\s*?=(["\'])(.+?)\1[^>]*?>/ui'; \1 ссылка на первую переменнуюю Переменная в (["\']). (.+?) - это вторая переменная**/

            preg_match_all('/<a\s*?[^>]*?href\s*?=(["\'])(.+?)\1[^>]*?>/ui', $content, $links); // все вхождения регулярного выражения
