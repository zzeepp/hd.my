...
//            }
    ...
    protected function editData($returnId = false)
    {
        ...
        {
            $id = is_numeric($_POST[$this->columns['id_row']]) ? $this->cleanNum($_POST[$this->columns['id_row']]) : $this->clearStr($_POST[$this->columns['id_row']]);
    ...
    protected function clearPostFields($settings,
                                       &$arr = [])
    {
        if(!$arr) $arr = &$_POST;
        if(!$settings) $settings = Settings::instance();

        $id = isset($this->columns['id_row']) ? $_POST[$this->columns['id_row']] : false;

        $validate = $settings::get('validations');

        if(!$this->translate) $this->translate = $settings::get('translate');

        foreach($arr as $key => $item)
        {
            if(is_array($item)) $this->clearPostFields($settings, $item);
            else
            {
                if(is_numeric($item)) $arr[$key] = $this->cleanNum($item);
    ...
    protected function emptyFields($str,
                                   $answer,
                                   $arr = [])
    {
        if(empty($str))
    ...
    protected function countChar($str,
                                 $counter,
                                 $answer,
                                 $arr)
    {
    ...
    protected function sortingFiles($fileArr,
                                    $arr)
    {
        $res = [];
        ...
        if($manyToMany)
        {
            foreach($manyToMany as $mTable => $tables)
            {
                ...
                if($targetKey !== false)
                {
                    ...
                    if($this->data) //если заполнено свойство data значит режим редактирования
                    {
                    ...
                    elseif($orderData['parent_id']) //вложенный фильтр
                    {
                                ...
                                'join'           => [
                                    $tables[$otherKey] => [
                                        'fields' => [
                                            ...
                                            $orderData['name']],
                                        'on'     => [
                                            $parentOrderData['columns']['id_row'],
                                            $orderData['parent_id']]
                                    ]
                                ],
                                'join_structure' => true
    ...
    protected function chechManyToMany($settings = false)
    {
        if(!$settings)
        {
            $settings = $this->settings ?: Settings::instance();
    ...
    protected function createForeinProperty($arr,
                                            $rootItems)
    {
        ...
        $orderDate = $this->createOrderData($arr['REFERENCED_TABLE_NAME']);
        $where     = [];
        $operand   = [];
    ...
    protected function createForeignData($settings = false)
    {
        ...
        elseif($this->columns['parent_id'])
        {
            ...
            $this->createForeinProperty($arr, $rootItems);
