...
abstract class BaseModelMethods
{
    ...
    protected function createFields($set,
                                    $table = false,
                                    $join = false)
    {
        ...
        else
        {
            ...
            foreach($set['fields'] as $field)
            {
                ...
                if($field || $field === null)
                {
                    ...
                    if($join && $join_structure)
                    {
                        ...
                        else $fields .= $concat_table . $field . ' as TABLE' . $alias_table . 'TABLE_' . $field . ',';
                    }
                    else
                    {
            ...
            if(!$id_field && $join_structure)
            {
                ...
                else $fields .= $concat_table . $this->tableRows[$alias_table]['id_row'] . ',';
            }
        }
    ...
    protected function createOrder($set,
                                   $table = false)
    {
                                   ...
                                   $instraction = 'WHERE')
    {
        ...
        if(!isset($set['where'])) return $where;

        if(is_string($set['where'])) return $instraction . ' ' . trim($set['where']);
        ...
        if(is_array($set['where']) && !empty($set['where']))
        {
            ...
            foreach($set['where'] as $key => $item)
            {
                ...
                {
                    if(is_string($item) && strpos($item, 'SELECT') === 0) $in_str = $item;
                    else
                    {
                ...
                else
                {
                    ...
                    else $where .= $table . $key . $operand . "'" . addslashes($item) . "' " . $condition;
                }
            }
        ...
        if(isset($set['join']))
        {
            ...
            foreach($set['join'] as $key => $item)
            {
                ...
                if(isset($item['on']) && $item['on'])
                {
                    ...
                    else $group_condition = $item['group_condition'] ? $item['group_condition'] : 'AND';
    ...
    protected function createInsert($fields,
                                    $files,
                                    $except)
    {
        $insert_arr           = [];
        ...
        if(is_int($array_type))
        {
            ...
            foreach($fields as $i => $item)
            {
                ...
                {
                    if($except && in_array($row)) continue;
                    ...
                    else $insert_arr['values'] .= "'" . addslashes($value) . "',";
    ...
    protected function createUpdate($fields,
                                    $files,
                                    $except)
    {
        $update = '';
        if($fields)
        {
            foreach($fields as $row => $value)
            {
                ...
                if(in_array($value, $this->sqlFunc)) $update .= $value . ',';
                elseif($value === NULL || $value === 'NULL') $update .= "NULL" . ',';
                else $update .= "'" . addslashes($value) . "',";
            }
        }
    ...
    protected function joinStructure($res,
                                     $table)
    {
        $join_arr = [];
    ...
    protected function createTableAlias($table)
    {
        ...
        else $arr['alias'] = $arr['table'] = $table;
